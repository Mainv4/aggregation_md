...
def loadUnivers(traj_file, top_file):
    ...
    try:
        if traj_file.split('.')[-1] == 'lammpstrj':
            ...
            print("Please consider an amber trajectory file")
            sys.exit()
...
def computeAgg(atoms, epsilons_matrix, atom_types, n_atoms, last, frame_current):
    ...
    for i in range(n_atoms):
        ...
        resname = atom_types[i].split(' ')[1]
        #print("resname : {}".format(resname))
        #print("indice : {}".format(i))
        mask = np.ma.masked_equal(atoms_all[pairs[i][:, 0]].resnames, atoms_all[pairs[i][:, 1]].resnames).mask
        pairs[i] = pairs[i][mask]
        try:
            mask = np.ma.masked_equal(atoms_all[pairs[i][:, 0]].resnames, resname).mask
        except IndexError:
            pass
        pairs[i] = pairs[i][mask]
        #print("atoms_all[pairs[i]].resnames : {}".format(atoms_all[pairs[i]].resnames))
        #print()
        #print()
        #print()
    ...
    for i in range(n_atoms):
        for j in range(i + 1, n_atoms):
            ...
            mask = np.ma.masked_not_equal(atoms_all[pairs[indice][:, 0]].resnames, atoms_all[pairs[indice][:, 1]].resnames).mask
            pairs[indice] = pairs[indice][mask]
            # We create a mask to remove the pairs that are not relevant
            # (all A[i]: (A[i,0] != resname_i and A[i,1] != resname_j) or (A[i,0] != resname_j and A[i,1] != resname_i))
            # Attention, the two conditions have to be evaluated at the same time
            #print()
            try:
                mask_ij_res_i_res_j = np.ma.masked_equal(atoms_all[pairs[indice][:, 0]].resnames, resname_i).mask
                mask_ij_res_j_res_i = np.ma.masked_equal(atoms_all[pairs[indice][:, 0]].resnames, resname_j).mask
                mask_ji_res_i_res_j = np.ma.masked_equal(atoms_all[pairs[indice][:, 1]].resnames, resname_i).mask
                mask_ji_res_j_res_i = np.ma.masked_equal(atoms_all[pairs[indice][:, 1]].resnames, resname_j).mask
                mask_ij = (mask_ij_res_i_res_j) & (mask_ji_res_j_res_i) | (mask_ij_res_j_res_i) & (mask_ji_res_i_res_j)
                pairs[indice] = pairs[indice][mask_ij]
            except IndexError:
                pass
            #print("atoms_all[pairs[i]].resnames : {}".format(atoms_all[pairs[indice]].resnames))
            #print()
            #print()
            #print()
    ...
    for i in range(n_atoms):
        resname = atom_types[i].split(' ')[1]
        str_type.append("x.resnames == '" + resname + "'")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Calculate the distribution of aggregates composed by given atoms of given resnames thanks by distances evaluation from Amber trajectory file")
    ...
    if args.compute == 'yes':
        ...
        for i in range(len(atom_types)):
            ...
            count_atom_type += 1
        print()
        if n_atoms > count_resname:
            print("You cannot consider more resnames than the number of resnames in the system")
            sys.exit()
        ...
        if len(epsilons) != n_epsilons_expected:
            ...
            for i in range(args.n_atoms):
                ...
                print()
            print("Where e(i) is the distance given at position i in the list of distances and corresponds to the distance between the atoms of resnames specified in the title of the column and the row")
            sys.exit()
